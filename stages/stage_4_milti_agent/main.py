...
async def check_routing(state: LegalState) -> dict:
    ...
    needs_tax = bool(parsed.get("needs_tax", True)) or any(
        kw in question_lower for kw in ["tax", "irs", "thuế"]
    )
...
QUESTION = (
    "Tôi là CEO của một công ty công nghệ ở Việt Nam. Chúng tôi đang xem xét một hợp đồng lớn với một đối tác quốc tế, nhưng tôi lo ngại về các rủi ro pháp lý liên quan đến thuế, tuân thủ quy định, và bảo vệ dữ liệu cá nhân. "
)
...
async def main():
    ...
    print("  3. Tax + Compliance + Privacy specialists run IN PARALLEL when needed")
